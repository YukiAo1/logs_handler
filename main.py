...
def _clean_logs():
    ...
    for f in glob.glob(os.path.join(logs_dir, '*.log')):
        try:
            os.remove(f)
            print(f'  已清理日志: {os.path.basename(f)}')
        except OSError as e:
            print(f'  清理失败 {os.path.basename(f)}: {e}')
    print('日志已清理')
...
def main():
    ...
    if rg_avail():
        print('  bin/rg.exe 已就绪 (主搜索引擎: 大文件自动启用rg加速)')
    else:
        reason = rg_reason()
        if reason:
            print(f'  bin/rg.exe 不可用: {reason}')
        else:
            print('  bin/rg.exe 未安装 (放rg.exe到bin/目录启用rg搜索引擎)')
