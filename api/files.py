...
_loading_state: dict = {'running': False, 'current': 0, 'total': 0, 'path': ''}
...
def _loading_progress(current, total, path=''):
    global _loading_state
    _loading_state['current'] = current
    _loading_state['total'] = total
    if path:
        _loading_state['path'] = path
...
@router.post('/open')
async def open_files(body: OpenRequest):
    global _loading_state
    ...
    if new_paths:
        _loading_state = {'running': True, 'current': 0, 'total': len(new_paths), 'path': new_paths[0] if new_paths else ''}
    ...
    search_logger.info(
        f"open_files | paths={len(resolved)} new={len(new_paths)} "
        f"total_files={len(_file_indexes)} total_lines={total_lines} ms={elapsed*1000:.0f}")
...
async def upload_files(files: list[UploadFile] = File(...)):
    ...
    if not saved_paths:
        raise HTTPException(status_code=400, detail='未找到有效的 .log 或 .txt 文件')
    result = await open_files(OpenRequest(paths=saved_paths))
    return result
