const Files = {
  ...
  _pollProgress() {
    ...
    API.get('/api/files/load-progress').then(data => {
      if (!data.running) {
        this._progressTimer = setTimeout(() => this._pollProgress(), 500);
        return;
      }
      const bar = document.getElementById('loadingProgressBar');
      const count = document.getElementById('loadingCount');
      ...
      if (count) count.textContent = pct > 0 ? `${data.current} / ${data.total}` : '';
      if (text) text.textContent = data.current > 0 && data.path
        ? `索引中: ${data.path.split(/[/\\]/).pop()}`
        : data.current > 0 ? '索引中...' : '正在上传...';
      this._progressTimer = setTimeout(() => this._pollProgress(), 300);
    }).catch(() => {
      this._progressTimer = setTimeout(() => this._pollProgress(), 500);
    });
  },
