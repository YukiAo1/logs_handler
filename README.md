...
基于 FastAPI + SQLite 的本地日志分析工具，提供日志加载、多级筛选、规则分组管理、**规则复选OR检索**、经典场景记录、双文件对比、结果导出、**自定义皮肤**等功能，支持 PyInstaller 打包为独立 exe。

## 功能总览

| 模块 | 功能 |
|------|------|
| 📂 文件管理 | 拖拽/选择/文件夹加载 .log/.txt，mmap 行偏移索引，并行建索引，loading 进度条 |
| 🔍 搜索筛选 | 级别/时间/PID/TID/Tag/关键字(正则)多级组合筛选，**规则多选OR**一键搜索，**S/E快速填时**，⚡**rg/Python引擎可选** |
...
| ⚡ 性能优化 | mmap 行偏移索引、**rg批量并行扫描**、**结果缓存(翻页秒开)**、**全量搜索短路(862万行5ms)** |
...
# 输出: dist/logs_handler.exe (约 18MB)
```

### 安装 ripgrep（推荐）

从 [GitHub Releases](https://github.com/BurntSushi/ripgrep/releases) 下载 `rg.exe`，放入项目 `bin/` 目录即可启用 ⚡ **RG 搜索引擎**，大文件搜索可提速 10~20 倍。
...
| 模式 | 说明 | 适用场景 |
|---|---|---|
| 🤖 **智能**（默认） | ≥500MB 自动启用 rg | 普通用户推荐 |
| ⚡ **RG** | 强制使用 ripgrep | 大文件+复杂正则 |
| 🐍 **Python** | 强制使用 Python mmap | 小文件/无 rg.exe |

- 无 `rg.exe` 时，⚡RG 和 🤖智能 自动禁用，强制 🐍Python
- 切换引擎即时保存偏好，重启后保持
...
| 场景 | 之前 | 现在 | 加速 |
|---|---|---|---|
| 级别过滤 E | ~28s | **~3-5s**（rg 批量） | **5-10x** |
| 级别+关键字 | ~28s | **~4-6s**（rg+post-filter） | **5-7x** |
| 翻页 | ~21s | **~0.01s**（缓存命中） | **~1000x** |
| 全量无过滤 | ~30s | **~5ms**（短路优化） | **6000x** |
| 加载 97 个文件 | ~3.2s | ~3.2s（并行索引） | 相当 |
...
文件加载过程中显示全屏进度条（索引行号、建时间索引），完成后在下方显示文件列表，包含文件大小、行数、时间范围。
...
搜索耗时超过 300ms 时自动弹出「查询中...」遮罩（可配置），完成后自动关闭。
...
│   ├── search.py        # 搜索筛选（支持多 rule_ids + engine参数）
│   ├── compare.py       # 日志对比
│   ├── export.py        # 结果导出
│   └── scenarios.py     # 经典场景 CRUD + 导入导出 + 批量匹配
├── storage/
│   ├── database.py      # SQLite 连接管理 + 建表 + 自动迁移
